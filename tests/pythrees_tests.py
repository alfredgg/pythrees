...
class TestMovement(TestCase):
    def test_up_00 (self):
        ini = [[0], [1], [0]]
        current = pythrees.move_tiles(ini, pythrees.UP_MOVEMENT)
        expected = [[1], [0], [0]]
        self.assertEqual(expected, current)

    def test_right_00 (self):
        ini = [[1,0,0]]
        current = pythrees.move_tiles(ini, pythrees.RIGHT_MOVEMENT)
        expected = [[0,0,1]]
        self.assertEqual(expected, current)

    def test_down_00 (self):
        ini = [[1,0,0], [0,0,0], [0,0,0]]
        current = pythrees.move_tiles(ini, pythrees.DOWN_MOVEMENT)
        expected = [[0,0,0], [0,0,0], [1,0,0]]
        self.assertEqual(expected, current)
    
    def test_left_00 (self):
        ini = [[0,0,1], [0,0,0], [0,0,0]]
        current = pythrees.move_tiles(ini, pythrees.LEFT_MOVEMENT)
